...
// SproutsExtreme message start (switch from Bitcoin's in v0.2)
static unsigned char pchMessageStartBitcoin[4] = { 0xcf, 0xbe, 0xb4, 0xfc };
static unsigned char pchMessageStartSproutsExtreme[4] = { 0xfc, 0xcf, 0xfc, 0xcf };
...
void GetMessageStart(unsigned char pchMessageStart[], bool fPersistent)
{
    if (fTestNet)
        ...
        memcpy(pchMessageStart, (fPersistent || GetAdjustedTime() > nMessageStartSwitchTime)? pchMessageStartSproutsExtreme : pchMessageStartBitcoin, sizeof(pchMessageStartSproutsExtreme));
}
