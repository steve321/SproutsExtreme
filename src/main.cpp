...
const string strMessageMagic = "SproutsExtreme Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // SproutsExtreme: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // SproutsExtreme: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // SproutsExtreme: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// SproutsExtreme: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // SproutsExtreme: subsidy is cut in half every 16x multiply of difficulty
...
// SproutsExtreme: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge)
{
    static int64 nRewardCoinYear = 7300 * CENT;  // creation amount per coin-year
    int64 nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear;

    strMotivational = "Extreme!";
    if (fDebug && GetBoolArg("-printcreation"))
...
// SproutsExtreme: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // SproutsExtreme: target change every block
    // SproutsExtreme: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // SproutsExtreme: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // SproutsExtreme: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // SproutsExtreme: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // SproutsExtreme: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // SproutsExtreme: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // SproutsExtreme: track money supply and mint amount info
    ...
    // SproutsExtreme: fees are not collected by miners as in bitcoin
    // SproutsExtreme: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// SproutsExtreme: total coin age spent in transaction, in the unit of coin-days.
...
// SproutsExtreme: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // SproutsExtreme: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // SproutsExtreme: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // SproutsExtreme: record proof-of-stake hash value
    ...
    // SproutsExtreme: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // SproutsExtreme: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // SproutsExtreme: coinbase output should be empty if proof-of-stake block
    ...
    // SproutsExtreme: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // SproutsExtreme: check that the block satisfies synchronized checkpoint
    ...
    // SproutsExtreme: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // SproutsExtreme: check proof-of-stake
    ...
    // SproutsExtreme: verify hash target and signature of coinstake tx
    ...
    // SproutsExtreme: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // SproutsExtreme: getblocks may not obtain the ancestor block rejected
    ...
    // SproutsExtreme: if responsible for sync-checkpoint send it
...
// SproutsExtreme: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "SproutsExtreme", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "SproutsExtreme", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // SproutsExtreme: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // SproutsExtreme: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // SproutsExtreme: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // SproutsExtreme: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // SproutsExtreme: should not enter safe mode for longer invalid chain
    // SproutsExtreme: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 15) && !fTestNet)
    {
        nPriority = 100;
        strStatusBar = "SproutsExtreme";
    }

    // SproutsExtreme: if detected invalid checkpoint enter safe mode
    ...
    // SproutsExtreme: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // SproutsExtreme: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // SproutsExtreme: record my external IP reported by peer
        ...
        // SproutsExtreme: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // SproutsExtreme: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // SproutsExtreme: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // SproutsExtreme: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // SproutsExtreme: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // SproutsExtreme: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
