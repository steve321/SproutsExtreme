...
static const int STAKE_MIN_AGE = 60 * 60 * 24 * 4; // minimum age for coin age 4 days
...
public:
    ...
    {
        // SproutsExtreme: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // SproutsExtreme: get transaction coin age
...
public:
    ...
    // SproutsExtreme: block signature - signed by coin base txout[0]'s owner
    ...
    // SproutsExtreme: two types of block: proof-of-work or proof-of-stake
    ...
    // SproutsExtreme: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // SproutsExtreme: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // SproutsExtreme: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // SproutsExtreme: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // SproutsExtreme: block index flags
    enum  
    {
